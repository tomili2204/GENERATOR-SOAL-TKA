...
import { selectThemeForGeneration } from "@/lib/generator/theme-selector";
...
async function main() {
  ...
  const smpOnlyThemes = allThemes.filter((t: any) => {
    const list = (t.jenjangCocok as string[]) || [];
    return !list.includes("SD/MI");
  });

  console.log(`[2] Jumlah Tema Khusus SMP ke atas: ${smpOnlyThemes.length} tema:`);
  smpOnlyThemes.forEach((t: any) => {
    console.log(`    - ${t.namaTema} -> [${(t.jenjangCocok as string[]).join(", ")}]`);
  });
  ...
  const matchedSmpOnly = expectedSmpOnly.every((name) =>
    smpOnlyThemes.some((t: any) => t.namaTema === name)
  );
